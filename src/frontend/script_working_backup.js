...
class TradingChartApp {
    ...
    drawFVGs() {
        ...
        fvgs.forEach((fvg, index) => {
            ...
            // 計算 FVG 的時間範圍
            const startTime = fvg.time; // FVG 開始時間
            
            // 計算結束時間：限制為最多20個K線
            const maxExtendBars = 20;
            const timeStepSeconds = this.getTimeStepByTimeframe(this.currentTimeframe);
            const maxExtendTime = startTime + (maxExtendBars * timeStepSeconds);
            ...
            // 調試信息（第一個FVG）
            if (index === 0) {
                console.log(`FVG像素感知: height=${height.toFixed(2)}, pixels=${pxH || 'null'}, lines=${lineCount}`);
            }
